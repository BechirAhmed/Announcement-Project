...
class ProfilesController extends Controller
{
    ...
    public function update($username, Request $request)
    {
        ...
        $input = Input::only('theme_id', 'location', 'bio', 'twitter_username', 'github_username', 'facebook_username', 'avatar_status');
    ...
    public function coverUpload()
    {
        if (Input::hasFile('file')) {
            ...
            // Save the file to the server
            Image::make($cover)->resize(600, 150)->save($save_path.$filename);

            // Save the public image path
            $currentUser->profile->cover = $public_path;
            $currentUser->profile->save();
